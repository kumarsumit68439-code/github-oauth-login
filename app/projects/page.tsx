...
export default function ProjectsPage() {
  ...
  const applyPromptTemplate = () => {
    ...
    setDraftHtml(`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>${draftTitle.replace(/</g, "")}</title>
  <style>
    body { margin:0; font-family: system-ui,sans-serif; background:#0b1220; color:#e2e8f0; }
    header { padding: 1.25rem 1.5rem; border-bottom: 1px solid #1e293b; display:flex; justify-content:space-between; align-items:center; }
    main { max-width: 900px; margin: 0 auto; padding: 3rem 1.5rem; }
    h1 { font-size: clamp(2rem, 5vw, 3rem); line-height:1.15; margin:0 0 1rem;
      background: linear-gradient(90deg,#a78bfa,#f472b6); -webkit-background-clip:text; color:transparent; }
    p { color:#94a3b8; font-size:1.1rem; line-height:1.6; max-width: 36rem; }
    .btn { display:inline-block; margin-top:1.5rem; padding:0.75rem 1.35rem; border-radius:999px;
      background:linear-gradient(90deg,#7c3aed,#db2777); color:#fff; text-decoration:none; font-weight:600; }
    .grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(200px,1fr)); gap:1rem; margin-top:2.5rem; }
    .card { background:#0f172a; border:1px solid #1e293b; border-radius:1rem; padding:1.25rem; }
  </style>
</head>
<body>
  <header><strong>${draftTitle.replace(/</g, "")}</strong><span style="color:#64748b">Live</span></header>
  <main>
    <h1>${p.replace(/</g, "").slice(0, 80)}</h1>
    <p>Built with the project builder. Edit HTML on the left, preview on the right, then Publish for a public URL.</p>
    <a class="btn" href="#">Get started</a>
    <div class="grid">
      <div class="card"><h3>Fast</h3><p style="margin:0;font-size:0.9rem">Instant preview</p></div>
      <div class="card"><h3>Publish</h3><p style="margin:0;font-size:0.9rem">Share /p/id link</p></div>
      <div class="card"><h3>Code</h3><p style="margin:0;font-size:0.9rem">Full HTML/CSS/JS</p></div>
    </div>
  </main>
</body>
</html>`);
    ...
    try {
      const res = await fetch("/api/projects", {
        ...
        body: JSON.stringify({
          ...
          html: draftHtml,
          files: [
            { id: "1", name: "index.html", language: "html", content: draftHtml },
          ],
        }),
      });
      const data = await res.json();
  ...
  const publishFromEditor = async () => {
    ...
    try {
      ...
      const title = prompt("Project title", draftTitle) || "Untitled project";
      const res = await fetch("/api/projects", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const loadIntoBuilder = async (id: string) => {
    try {
      ...
      if (!res.ok) {
        // fallback open public page only
      ...
      if (data.html) {
        ...
        setDraftTitle(data.title || "Project");
        setTab("build");
      }
    } catch {
      window.open(`${origin}/p/${id}`, "_blank");
  ...
  return (
    <AuthGuard>
      <div style={{ minHeight: "calc(100vh - 56px)", background: "#070b14", color: "#e2e8f0" }}>
        ...
        {tab === "build" ? (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", minHeight: "calc(100vh - 120px)" }} className="builder-grid">
            ...
            <div style={{ borderRight: "1px solid #1e293b", display: "flex", flexDirection: "column", minHeight: 480 }}>
              <div style={{ padding: 12, borderBottom: "1px solid #1e293b", display: "flex", flexDirection: "column", gap: 8 }}>
                ...
                <textarea
                  ...
                  placeholder="Describe your app… e.g. dark SaaS landing with pricing cards"
                  rows={2}
              ...
              <div style={{ padding: "8px 12px", fontSize: 11, color: "#64748b" }}>HTML / CSS / JS (index)</div>
              <textarea
                value={draftHtml}
                ...
                return (
                  ...
                  >
                    <iframe
                      ...
                      style={{ width: "100%", height: 160, border: "none", background: "#fff", pointerEvents: "none" }}
                    />
